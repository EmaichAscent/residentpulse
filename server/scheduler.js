...
async function sendApproachingRoundReminders() {
  ...
  const day7Rounds = await db.all(
    `SELECT id, client_id, round_number, scheduled_date FROM survey_rounds
     WHERE status = 'planned'
       AND admin_reminder_7_sent = false
       AND is_test = FALSE
  ...
  const day1Rounds = await db.all(
    `SELECT id, client_id, round_number, scheduled_date FROM survey_rounds
     WHERE status = 'planned'
       AND admin_reminder_1_sent = false
       AND is_test = FALSE
...
async function cleanupAbandonedSignups() {
  ...
  for (const client of abandoned) {
    ...
    await db.run("DELETE FROM users WHERE client_id = ?", [client.id]);
