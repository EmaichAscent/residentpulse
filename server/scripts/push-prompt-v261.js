...
/**
 * Push the V2.6.1 board interview prompt to the live `settings` row that
 * powers every board-member chat in production. This is the
 * "make-it-live" step after the V2.6.1 prompt-text changes ship in code.
 *
 ...
 *      labeled "Auto-save (pre-V2.6.1)" so the operator can roll back
 *   3. Updates settings.value to V2_SYSTEM_PROMPT (V2.6.1)
 *   4. Inserts another prompt_versions row labeled "V2.6.1" so the
 ...
 *     production URL).
 *   • If the live value is ALREADY V2.6.1 (re-run protection), exits
 ...
 *   node server/scripts/push-prompt-v261.js
 *
 *   # Apply:
 *   node server/scripts/push-prompt-v261.js --apply
 */

import pg from "pg";
import { V2_SYSTEM_PROMPT, V2_SYSTEM_PROMPT_V26 } from "../prompts/defaults.js";
const { Client } = pg;
...
async function main() {
  ...
  if (bytesEqual(liveText, V2_SYSTEM_PROMPT)) {
    console.log(`\n✓ Live value is ALREADY V2.6.1 — nothing to do.`);
    await client.end();
    return;
  }
  ...
  let autoSaveLabel = "Auto-save (pre-V2.6.1)";
  let autoSaveNote = "Live value snapshot before push-prompt-v261.js applied V2.6.1.";
  if (bytesEqual(liveText, V2_SYSTEM_PROMPT_V26)) {
    autoSaveLabel = "V2.6 (pre-V2.6.1 snapshot)";
    autoSaveNote = "Live value matched the frozen V2.6 default exactly when V2.6.1 was pushed.";
    console.log(
      `\nLive value matches V2.6 default exactly — auto-save labeled "${autoSaveLabel}".`
    );
  } else {
    console.log(
      `\nLive value does NOT match V2.6 default exactly — somebody edited it via the SuperAdmin UI. ` +
  ...
  try {
    ...
    await client.query(
      ...
      [PROMPT_KEY, liveText, autoSaveLabel, autoSaveNote, oldVersionNumber, "push-prompt-v261.js"]
    );
    console.log(`  [APPLY] auto-saved OLD value as v${oldVersionNumber} ("${autoSaveLabel}")`);

    // 2. Update settings.value to V2.6.1
    await client.query(`UPDATE settings SET value = $1 WHERE key = $2 AND client_id IS NULL`, [
      V2_SYSTEM_PROMPT,
      PROMPT_KEY,
    ]);
    console.log(
      `  [APPLY] updated settings.${PROMPT_KEY} to V2.6.1 (${V2_SYSTEM_PROMPT.length} chars)`
    );
    ...
    await client.query(
      ...
      [
        ...
        V2_SYSTEM_PROMPT,
        "V2.6.1",
        "Vary worked-example pivot phrasing so the model stops reusing the same stock pivot string verbatim across interviews. Pushed via push-prompt-v261.js.",
        newVersionNumber,
        "push-prompt-v26.js",
      ]
    );
    console.log(`  [APPLY] saved NEW value as v${newVersionNumber} ("V2.6.1")`);

    await client.query("COMMIT");
    console.log(`\n✓ COMMITTED — V2.6.1 is now live.`);
  } catch (err) {
