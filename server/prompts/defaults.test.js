...
describe("V2 system prompt — V2.0/V2.1/V2.2/V2.3/V2.4 frozen for migration matching", () => {
  ...
  it("current V2_SYSTEM_PROMPT (V3.2) differs from all frozen versions", () => {
    ...
    expect(V2_SYSTEM_PROMPT).not.toEqual(V3_1_SYSTEM_PROMPT);
  });
});
...
describe("V3.1 — frozen for V3.2 derivation", () => {
  it("V3.1 is longer than V3.0 (added drill section) but bounded", () => {
    expect(V3_1_SYSTEM_PROMPT.length).toBeGreaterThan(V3_0_SYSTEM_PROMPT.length);
    expect(V3_1_SYSTEM_PROMPT.length).toBeLessThan(10000);
  });

  it("V3.1 still contains its drill keyword list (V3.2 will replace)", () => {
    expect(V3_1_SYSTEM_PROMPT).toContain("## Drill before pivoting");
    expect(V3_1_SYSTEM_PROMPT).toMatch(/meltdown/);
    expect(V3_1_SYSTEM_PROMPT).toMatch(/blew up/);
    expect(V3_1_SYSTEM_PROMPT).toMatch(/lost their cool/);
    expect(V3_1_SYSTEM_PROMPT).toContain(V3_1_DRILL_SECTION);
  });
});

describe("V3.2 — simpler principle-based drill (replaces V3.1 keyword list)", () => {
  it("is SHORTER than V3.1 (the principle is simpler than the keyword list)", () => {
    expect(V2_SYSTEM_PROMPT.length).toBeLessThan(V3_1_SYSTEM_PROMPT.length);
    // V3.2 ≈ 7.9k (V3.0 6.9k + simpler drill ~1k vs V3.1's ~1.5k drill)
  ...
  it("V3.2 drill section uses the new principle-based header", () => {
    expect(V2_SYSTEM_PROMPT).toContain("## Drill on specifics, don't broaden");
    const drillIdx = V2_SYSTEM_PROMPT.indexOf("## Drill on specifics");
  ...
  it("V3.2 STRIPS the V3.1 keyword-trigger list entirely", () => {
    // The V3.1 keyword list was the source of the "come apart" miss.
    // V3.2 replaces it with one principle, so these specific keywords
    // shouldn't appear in the drill section anymore.
    expect(V2_SYSTEM_PROMPT).not.toContain("## Drill before pivoting");
    expect(V2_SYSTEM_PROMPT).not.toMatch(/Drill triggers — any of these/);
    expect(V2_SYSTEM_PROMPT).not.toMatch(/"meltdown", "blew up"/);
    // The V3.1 drill section as a verbatim block should be gone.
    expect(V2_SYSTEM_PROMPT).not.toContain(V3_1_DRILL_SECTION);
  });

  it("V3.2 contains the principle: drill on specifics, don't broaden", () => {
    expect(V2_SYSTEM_PROMPT).toMatch(
      /When the resident names a specific event, behavior, or named person/
    );
    expect(V2_SYSTEM_PROMPT).toMatch(/Don't broaden to abstract themes/);
    // The new explicit anti-pattern from the V3.1 production transcript
    expect(V2_SYSTEM_PROMPT).toMatch(/Re-broadening to abstract themes IS a pivot/);
  });

  it("V3.2 keeps the four useful drill questions (carried over from V3.1)", () => {
    expect(V2_SYSTEM_PROMPT).toMatch(/What happened\?/);
  ...
  it("V3.2 keeps the hard-rule override from V3.1", () => {
    expect(V2_SYSTEM_PROMPT).toContain(V3_1_HARD_RULE_FOLLOWUPS);
    expect(V2_SYSTEM_PROMPT).not.toContain(V3_0_HARD_RULE_FOLLOWUPS);
  });

  it("V3.2 updates the Capture-only header to reference the new drill section", () => {
    expect(V2_SYSTEM_PROMPT).toContain(V3_2_CAPTURE_ONLY_HEADER);
    expect(V2_SYSTEM_PROMPT).not.toContain(V3_1_CAPTURE_ONLY_HEADER);
    expect(V2_SYSTEM_PROMPT).not.toContain(V3_0_CRITICAL_SIGNALS_HEADER);
  });

  it("V3.2 still contains the dissolution / legal threats capture-only content", () => {
    expect(V2_SYSTEM_PROMPT).toMatch(/DISSOLUTION of the association/);
  });

  it("V3_2_DRILL_SECTION export is present in V3.2 verbatim", () => {
    expect(V2_SYSTEM_PROMPT).toContain(V3_2_DRILL_SECTION);
  });
